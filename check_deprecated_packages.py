...
# The below function is used to create a 2D list with each row and column values
def create_output_csv_file(input_file_name, package_name, package_version, bear_token):
    with open(input_file_name, mode ='r') as file:
        ...
        table_header_names = ["name", "repo", "version", "version_satisfied"]
        ...
        for values in input_csv_file:
            ...
            dependencies = get_package_object(values[1], bear_token)[2]
...
def check_deprecated_packages(input_file_name, package_name_with_version, bear_token):
